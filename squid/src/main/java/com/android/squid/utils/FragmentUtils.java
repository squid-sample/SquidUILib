...
public final class FragmentUtils {
        ...
        for (int i = fragments.size() - 1; i >= 0; --i) {
            ...
            if (fragment != null
                ...
                if (isInStack) {
                    Bundle args = fragment.getArguments();
                    if (args != null && args.getBoolean(ARGS_IS_ADD_STACK)) {
                        return getTopShowIsInStack(fragment.getChildFragmentManager(),
                                fragment, true);
                    }
                } else {
                    return getTopShowIsInStack(fragment.getChildFragmentManager(),
    ...
    public static Fragment[] weakReferencesFragment(@NonNull List<WeakReference<Fragment>>
                                                            weakReferences) {
        Fragment[] hide = new Fragment[weakReferences.size()];
        for (int i = 0; i < weakReferences.size(); i++) {
            hide[i] = weakReferences.get(i).get();
        }
        return hide;
    }
