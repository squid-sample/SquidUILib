package com.android.squid.utils;
...
public final class ThreadTools {
    ...
    public static <T> void executeByFixedAtFixRate(@IntRange(from = 1) final int size,
                                                   ...
                                                   final TimeUnit unit,
                                                   @IntRange(from = 1, to = 10) final int priority) {
        executeAtFixedRate(getPoolByTypeAndPriority(size, priority),
    ...
    private static <T> void execute(final ExecutorService pool, final Task<T> task, long delay,
    ...
    @SuppressWarnings({"checkstyle:IllegalCatch", "checkstyle:MemberName"})
    ...
    public abstract static class SingleTask<T> extends Task<T> {
        @Override
